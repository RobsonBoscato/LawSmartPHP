...
<html lang="en">
	...
	<body id="kt_body" style="background-image: url('../assets/media/misc/page-bg-dark.jpg')" class="dark-mode page-bg header-fixed header-tablet-and-mobile-fixed aside-enabled">
										...
										<div class="col-xl-4">
											...
											<div class="card card-xl-stretch mb-xl-8">
												...
												<div class="card-body p-0">
													...
													<?php
														$antecipadas = mysqli_query($lawsmt, "SELECT antecipadas.* FROM antecipadas 
																left join antecipadasDetalhes on antecipadasDetalhes.antecipada = antecipadas.id
																inner join operacoes on operacoes.id = antecipadasDetalhes.operacao
																and operacoes.confirmada = 1
														GROUP BY fornecedor ORDER BY valor DESC LIMIT 4");
														while($ant = mysqli_fetch_assoc($antecipadas)) {
															$fornecedores = mysqli_fetch_assoc(mysqli_query($lawsmt, "SELECT * FROM fornecedores WHERE id = '{$ant['fornecedor']}'"));
															$taa = mysqli_fetch_assoc(mysqli_query($lawsmt, "SELECT SUM(valor) as valor FROM antecipadas WHERE fornecedor = '{$ant['fornecedor']}'"));
													?>
														...
														<a href="#" class="card bg-info hoverable card-xl-stretch mb-5 mb-xl-8">
															...
															<div class="card-body">
																...
																<?php
																	...
																	$totaljuros = mysqli_fetch_assoc(mysqli_query($lawsmt, "SELECT SUM(descontoJuros) as valor FROM antecipadas")) or die(mysql_error());
																	$totaltaxas = mysqli_fetch_assoc(mysqli_query($lawsmt, "SELECT SUM(descontoTaxas) as valor FROM antecipadas")) or die(mysql_error());
																	$total_postergado = mysqli_fetch_assoc(mysqli_query($lawsmt, "select sum(postergacoesDetalhes.valorOriginal) as valor from operacoes
																	inner join postergacoesDetalhes on operacoes.id = postergacoesDetalhes.id_operacao
																	where operacoes.status = 5"));
																	$total_antecipado = mysqli_fetch_assoc(mysqli_query($lawsmt, "select distinct sum(antecipadasDetalhes.valorOriginal) as valor from antecipadas
																	...
																	"));
																?>
																<div class="text-white fw-bolder fs-2 mb-2 mt-5">R$ <?php echo number_format($total_antecipado['valor']+$total_postergado['valor'], 2, ',', '.'); ?></div>
													...
													<div>
														<a href="#" class="card bg-body hoverable card-xl-stretch mb-xl-8">
															...
															<div class="card-body">
																...
																<?php
																	$montantepositivo = mysqli_fetch_assoc(mysqli_query($lawsmt, "SELECT SUM(valor) as aportes FROM movimentacao WHERE tipo = 'aporte'"));
																	$montantenegativo = mysqli_fetch_assoc(mysqli_query($lawsmt, "SELECT SUM(valor) as retiradas FROM movimentacao WHERE tipo = 'retirada'"));
																...
																?>
																<?php  ?>
																<div class="text-gray-900 fw-bolder fs-2 mb-2 mt-5">R$ <?php echo number_format($valormontante - $totaldespesas['despesa'] + $total_postergado + $total_taxas + $total_operado, 2, ',', '.'); ?></div>
											...
											<div class="table-responsive">
												...
												<table class="table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3" id="kt_datatable_example_1">
													...
													<tbody>
														<?php
															$hoje = date("Y-m-d");
															$operacoes = mysqli_query($lawsmt, "SELECT * FROM operacoes WHERE vencimento >= '{$hoje}' and status != 4 and status != 1 ORDER BY vencimento DESC");
															while($ope = mysqli_fetch_assoc($operacoes)) {
																$fornecedor = mysqli_fetch_assoc(mysqli_query($lawsmt, "SELECT * FROM fornecedores WHERE cnpj = '{$ope['cnpj']}'"));
														?>
														<tr>
															<td>
		...
		<script>
			...
			<?php
				$total_operacoes_mes = mysqli_query($lawsmt, "select month(postergacoes.data) as mes, sum(postergacoesDetalhes.valorOriginal) as valor_total from operacoes
				...
				group by MONTH(antecipadas.data)") or die(mysqli_error());
				$rows = array();
 				while($r = mysqli_fetch_assoc($total_operacoes_mes)) {
				  $rows[$r['mes']] = $rows[$r['mes']] + $r['valor_total'];
				}
			 ?>
