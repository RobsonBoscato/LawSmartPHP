...
<html lang="en">
	...
	<body id="kt_body" style="background-image: url('../assets/media/misc/page-bg-dark.jpg')" class="dark-mode page-bg header-fixed header-tablet-and-mobile-fixed aside-enabled">
										...
										<div class="col-xl-6">
											...
											<a href="#" class="card bg-body hoverable card-xl-stretch mb-xl-8">
												...
												<div class="card-body">
													...
													<?php
														$montantepositivo = mysqli_fetch_assoc(mysqli_query($lawsmt, "SELECT SUM(valor) as aportes FROM movimentacao WHERE tipo = 'aporte'")) or die(mysql_error());
														$montantenegativo = mysqli_fetch_assoc(mysqli_query($lawsmt, "SELECT SUM(valor) as retiradas FROM movimentacao WHERE tipo = 'retirada' ")) or die(mysql_error());
														$total_postergado = mysqli_fetch_assoc(mysqli_query($lawsmt, "select sum(postergacoesDetalhes.juros) as juros, sum(postergacoesDetalhes.taxas) as taxas from operacoes
														inner join postergacoesDetalhes on operacoes.id = postergacoesDetalhes.id_operacao
														where operacoes.status = 5"));
														
														$mes_aplicado = mysqli_fetch_assoc(mysqli_query($lawsmt, "SELECT MONTH(movimentacao.data) as mes, YEAR(movimentacao.data) as ano FROM movimentacao WHERE tipo = 'aporte'")) or die(mysql_error());


														
														$total_antecipado = mysqli_fetch_assoc(mysqli_query($lawsmt, "select distinct sum(antecipadas.descontoJuros) as juros, sum(antecipadas.descontoTaxas) as taxas from antecipadas
										...
										<div class="col-xl-6">
											...
											<a href="#" class="card bg-info hoverable card-xl-stretch mb-5 mb-xl-8">
												...
												<div class="card-body">
													...
													<?php
														$totaloperado_query = mysqli_query($lawsmt, "SELECT antecipadas.descontoJuros as juros FROM antecipadas 
														...
														$total_operado = 0; 	
														while($row = mysqli_fetch_assoc($totaloperado_query)) {
															$total_operado += $row['juros'];
														}	


														$totaltaxas_query = mysqli_query($lawsmt, "SELECT descontoTaxas as taxas FROM antecipadas
														left join antecipadasDetalhes on antecipadasDetalhes.antecipada = antecipadas.id
														inner join operacoes on operacoes.id = antecipadasDetalhes.operacao
														  and operacoes.status = 5
														  group by antecipadas.id
														...
														$total_taxas = 0;  
														while($row = mysqli_fetch_assoc($totaltaxas_query)) {
															$total_taxas += $row['taxas'];
														}
														$totaldespesas = mysqli_fetch_assoc(mysqli_query($lawsmt, "SELECT SUM(valor) as despesa FROM movimentacao WHERE tipo = 'despesa'"));
														$total_postergado_query = mysqli_query($lawsmt, "select sum(postergacoesDetalhes.juros) as juros, sum(postergacoesDetalhes.taxas) as taxas from operacoes
														inner join postergacoesDetalhes on operacoes.id = postergacoesDetalhes.id_operacao
														where operacoes.status = 5");
														$total_postergado = 0;
														while($row = mysqli_fetch_assoc($total_postergado_query)) {
														  $total_postergado += $row['juros'] + $row['taxas'];
														}
														
														$total_lucro = $totaloperado['juros'] + $totaltaxas['taxas'] - $totaldespesas['despesa'] + $total_postergado['taxas'] + $total_postergado['juros'];
													?>
										...
										<div class="card-body p-0">
											<div class="d-flex flex-stack card-p flex-grow-1">
												...
												<div class="d-flex flex-column text-end">
													...
													<?php
														// echo "SELECT SUM(antecipadas.valor) as valor FROM antecipadas inner join antecipadasDetalhes on antecipadasDetalhes.antecipada = antecipadas.id inner join operacoes on antecipadasDetalhes.operacao = operacoes.id where operacoes.status = 5 and confirmada = 1";
														
														$totaloperado_query = mysqli_query($lawsmt, "SELECT antecipadas.valor as valor FROM antecipadas 
														...
														$conta_opera = mysqli_fetch_assoc(mysqli_query($lawsmt, "SELECT count(*) as totaloperacoes FROM operacoes"));

														$total_postergado = mysqli_fetch_assoc(mysqli_query($lawsmt, "select sum(postergacoesDetalhes.valorOriginal) as valor, 
														...
														$totalpago_query = mysqli_fetch_assoc(mysqli_query($lawsmt, "SELECT sum(valor) as total_pago FROM `boletos` where status = 9"));


														$total_operado = 0;
														while ($antecipacao = mysqli_fetch_assoc($totaloperado_query)) {
															...
															$total_operado = $total_operado + $antecipacao['valor'];
															
															// }
														}
													?>
													<span class="text-dark fw-bolder fs-2">R$ <?php echo number_format($valormontante + $totalpago_query['total_pago'] - $total_operado - $total_postergado['valor'], 2, ',', '.'); ?></span>
										...
										<div class="card-body p-0">
											<div class="d-flex flex-stack card-p flex-grow-1">
												...
												<div class="d-flex flex-column text-end">
													...
													<?php
														$totaloperado = mysqli_fetch_assoc(mysqli_query($lawsmt, "SELECT SUM(valor) as valor FROM operacoes"));
														$conta_opera = mysqli_fetch_assoc(mysqli_query($lawsmt, "SELECT count(*) as antecipadas FROM operacoes"));
														$total_postergado = mysqli_fetch_assoc(mysqli_query($lawsmt, "select sum(postergacoesDetalhes.valorOriginal) as valor, count(*) as total_titulos from operacoes
														inner join postergacoesDetalhes on operacoes.id = postergacoesDetalhes.id_operacao
														where operacoes.status = 5
														  and MONTH(operacoes.dataOPE) = MONTH(CURRENT_DATE)"));
													?>
													<span class="text-dark fw-bolder fs-2">R$ <?php echo number_format($total_postergado["valor"], 2, ',', '.'); ?></span>
													<span class="text-muted fw-bold mt-1">Em <?php echo $total_postergado["total_titulos"] ?> títulos</span>
												</div>
											</div>
										</div>
										<!--end::Body-->
										...
										<div class="card-body p-0">
											<div class="d-flex flex-stack card-p flex-grow-1">
												...
												<div class="d-flex flex-column text-end">
													...
													<?php
														$totaloperado = mysqli_fetch_assoc(mysqli_query($lawsmt, "SELECT SUM(antecipadasDetalhes.valorOriginal) as valor FROM antecipadas
														left join antecipadasDetalhes on antecipadasDetalhes.antecipada = antecipadas.id
														inner join operacoes on operacoes.id = antecipadasDetalhes.operacao
														and operacoes.status = 5"));
														$conta_opera = mysqli_fetch_assoc(mysqli_query($lawsmt, "SELECT count(*) as antecipadas FROM antecipadas"));
													?>
												...
												<div class="row">
													<div class="col-md-3">
														<select name="status" class="form-control">
															...
															<option value="4" <?php if($status == "4") { echo "selected"; } ?>>Assinado</option>
															<option value="1" <?php if($status == "1") { echo "selected"; } ?>>Não Assinado</option>
														</select>
													</div>
												...
												<table class="table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3"  id="kt_datatable_example_1">
													...
													<tbody>
														...
														<tr>
															...
															<td>
																<a href="" class="text-dark fw-bolder text-hover-primary fs-6"><?php if ($row['status'] == 4 || $row['real_antec_id']) {
																	?>
															...
															<td>
																<div class="fs-6 text-gray-800 fw-bolder"><?php if ($row['tipo'] != 'cliente') {
															...
															<td>
																...
																?>
															    	<!-- <a href="index.php?id=<?php echo $row['id_oper'] ?>&a=confirmar">
		...
		<script>
			...
			<?php
				$montante_aplicado_query = "select movimentacao.data as data, movimentacao.valor from movimentacao where movimentacao.tipo = 'aporte'";
				$result_montante_aplicado = mysqli_query($lawsmt, $montante_aplicado_query);
				$array_moves = array (null);
				while($rm = mysqli_fetch_assoc($result_montante_aplicado)) {
					$array_moves[$rm['data']] = $array_moves[$rm['data']]+$rm['valor'];
				}
				  
				$rentabilidade = mysqli_query($lawsmt, "select * from cdi where cdi.ano = YEAR(CURRENT_DATE)");
				$total_operacoes_mes = mysqli_query($lawsmt, "select month(postergacoes.data) as mes, sum(postergacoesDetalhes.juros) as valor_juros, sum(postergacoesDetalhes.taxas) as valor_taxas
				from operacoes
				...
				while($rm = mysqli_fetch_assoc($total_operacoes_mes)) {
				  $rows_month_law_smart[$rm['mes']] = $rows_month_law_smart[$rm['mes']]+$rm['valor_taxas']+$rm['valor_juros'];
				}

 				while($r = mysqli_fetch_assoc($rentabilidade)) {
				  $rows[$r['mes']] = $r['valor'];
				}

				$titulos_a_pagar_query = "select month(boletos.vencimento) as mes, count(boletos.id) as total from boletos where year(boletos.vencimento) = YEAR(CURRENT_DATE) group by month(boletos.vencimento)";
				$titulos_a_pagar = mysqli_query($lawsmt, $titulos_a_pagar_query);
				$titulos_a_pagar_array = array();
				while($rm = mysqli_fetch_assoc($titulos_a_pagar)) {
					$titulos_a_pagar_array[$rm['mes']] = $titulos_a_pagar_array[$rm['mes']]+$rm['total'];
				}
	
	
				$titulos_pagos_mes_query = "select month(boletos.data_ocorrencia) as mes, count(boletos.id) as total from boletos where year(boletos.data_ocorrencia) = YEAR(CURRENT_DATE) and boletos.status = 9 group by month(boletos.data_ocorrencia)";
				$titulos_pagos = mysqli_query($lawsmt, $titulos_pagos_mes_query);
				$titulos_pagos_array = array();
 				while($rm = mysqli_fetch_assoc($titulos_pagos)) {
			...
			Object.keys(agrupadoMensal).forEach((key, index) => {
				...
				const monthlyGrouped = Object.keys(agrupadoAplicado).map((localMonthApplied, index) => {
					if (localMonthApplied == 0) return null
					const monthAppliedStr = new String(localMonthApplied)
				...
				const percentageAtMonthList = monthlyGrouped.reduce((acc, cur) => acc+cur)
				const percentagAtMonth = percentageAtMonthList / monthlyGrouped.length
				data_cdi[month] = percentagAtMonth
			})
