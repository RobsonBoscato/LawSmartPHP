							...
							<div class="col-xl-12">
								...
								<div class="card card-xl-stretch mb-xl-8">
									...
									<div class="card-header border-0 pt-5">
										<div class="mb-3">
											<label for="exampleInputEmail1" class="form-label">CNPJ/RAZÃO SOCIAL</label>

											<select name="fornecedoresSelect" id="fornecedoresSelect" class="js-example-basic-single form-control">
	...
	<script>
		$(document).ready(function() {
			...
			$.ajax({
				...
				.done(function(response) { // Fix the parameter name from responce to response
					try {
						var data = JSON.parse(response);
						console.log(data);
						// Assuming data is an array of objects
						var select = $("#fornecedoresSelect"); // Replace with the actual ID or selector of your <select>

						$.each(data, function(index, item) {

							// Create a new <option> element and append it to the select
							select.append('<option value="' + item.cnpj + '">' + item.cnpj + ' - ' + item.razao + '</option>');
			...
			$('#fornecedoresSelect').on('change', function() {
				var cnpj = $(this).val();
				buscar(cnpj);
			});
		});
