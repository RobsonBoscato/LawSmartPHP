function buscar() {
    ...
    document.querySelector('#operacao-data').innerHTML = new Date(Date.now()).toLocaleDateString('pt-BR');
		...
		if (obj.fornecedor.operacoes.length > 0) {
			obj.fornecedor.operacoes.forEach((op, i) => {
				...
				//const checkAntecipar = (op.statusCliente == 'S') ? '<input data-bs-toggle="tooltip" data-bs-custom-class="tooltip-inverse" data-bs-placement="right" title="Selecionar" class="form-check-input operacoes-check" type="checkbox" data-attr="${i}" />' : '';
				/*
					&nbsp;
					<button data-attr="${i}" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-inverse" data-bs-placement="right" title="Postergar Operação" class="efetuar-postergacao btn btn-active-icon-primary btn-active-text-primary" style="margin:0; padding:0;">
				...
				*/
				const checkAntecipar = `<input data-bs-toggle="tooltip" data-bs-custom-class="tooltip-inverse" data-bs-placement="right" title="Selecionar" class="form-check-input operacoes-check" type="checkbox" data-attr="${i}" />`;
	...
	function exibeModalPostergacao(n) {
		...
		setTimeout(() => {
			...
			// se for ultimo item do array postergadas envia
			// senao 'proxima'
			const proxima = document.querySelector('#proxima-modal-postergacao');
			proxima.addEventListener('click', () => { fecha[0].click(); setTimeout(() => { exibeModalPostergacao(obj.fornecedor.postergadas[(idx + 1)]) }, 250); });
			...
			if (idx < obj.fornecedor.postergadas.length - 1) {
				...
				confirma.addEventListener('click', () => { fecha[0].click(); enviaPostergacao(); });
	...
	async function enviaPostergacao() {
		...
		let pcent = 0;
        console.log('enviando postergações >>>>>>>>>>>>.')
		exibeModal();
		...
		const posterg_id = postergacao_id.json()
		console.log('POSTERGACAO id RESULT', postergacao_id)
		console.log('posterg id is', posterg_id)
		const nosData = [];
	...
	function atualizaAntecipacao() {
		...
		if (antecipadas > 0) {
			...
			// 	const mensagem = document.querySelector('#antecipadas-modal-msg');
			// 	const titulo = document.querySelector('#antecipadas-titulo');
			// 	titulo.innerHTML = 'Efetuando Antecipações…';
			// 	let pcent = 0;
			// 	exibeModal();
			// 	const data = {
			// 		metodo: 'antecipadas',
