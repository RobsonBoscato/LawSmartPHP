...
if ($_GET) {
	...
	switch ($_GET['f']) {
		...
		case 'operacoes':
			...
			$stmt->close();
			echo json_encode($ope, JSON_FORCE_OBJECT);
			break;
		...
		case 'taxas':
			...
			$stmt->close();
			echo json_encode($txs, JSON_FORCE_OBJECT);
			break;
		...
		case 'PercentualLimiteUsado':
			...
			$sql2 = "SELECT SUM(valor) as totalUsado FROM operacoes WHERE cnpj = '{$cnpj}' AND status = 5 AND confirmada = 1 ";
			$stmt2 = prepared_query($con, $sql2, []);
			$ASQL = $stmt2->get_result()->fetch_all(MYSQLI_ASSOC);
			$stmt2->close();

			
			$totalUsado = $ASQL[0]['totalUsado'];
			$limite = $forn[0]['limite'];
