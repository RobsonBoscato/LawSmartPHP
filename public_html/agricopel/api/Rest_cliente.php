    ...
    if (!empty($response)) {
        ...
        if (isJson($response)) {
            ...
            foreach ($data['Retorno'] as $input_data) {
                ...
                $tipo = 'cliente';
                $fornecedorRazao = $input_data['clienteRazao'];
                $fornecedorCNPJ = $input_data['clienteCNPJ'];
                $fornecedorEmail = $input_data['clienteEmail'];
                $fornecedorTelefone = $input_data['clienteTelefone'];
                $fornecedorTaxaJuros =  str_replace([' ', ','], '', $input_data['clienteTaxaJuros']);
            
                // Removemos espaços em branco e vírgulas da string
                $fornecedorLimite = number_format(floatval($input_data['clienteLimite']), 2, '', '.');
       
                $fornecedorCustoBoleto = !empty($input_data['clienteCustoBoleto']) ? $input_data['clienteCustoBoleto'] : 0;
                $fornecedorTAC = !empty($input_data['clienteTAC']) ? $input_data['clienteTAC'] : 0;
                $fornecedorTED = !empty($input_data['clienteTED']) ? $input_data['clienteTED'] : 0;
