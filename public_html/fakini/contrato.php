...
//   function formataCpfCnpj(?string $str): string {
//     if ($str === null) {
//         return ''; // or another logic for handling null string
//     }

//     $formattedValue = ($str && (mb_strlen($str) == 14 || mb_strlen($str) == 13))
//         ? vsprintf('%s.%s.%s/%s-%s', str_split($str, 1))
//         : vsprintf('%s.%s.%s-%s', str_split($str, 1));
...
{
  if ($str === null) {
    // Handle the case where $str is null (you may want to throw an exception or return an error message)
    return '';
  }
  ...
  if (strlen($cep) !== 8) {
    // Handle invalid CEP length (you may want to throw an exception or return an error message)
    return $str;
  }

  return substr($cep, 0, 2) . '.' . substr($cep, 2, 3) . '-' . substr($cep, -3);
}
...
$sql = "SELECT descontoTaxas as taxas FROM antecipadas WHERE id = ?";
$stmt = prepared_query($con, $sql, [$id], 'i');
$antecip_taxas = $stmt->get_result()->fetch_all(MYSQLI_ASSOC)[0];
$stmt->close();
// var_dump($antecip);
...
foreach ($antecip as $a) {
  // echo "ANTECIP IS".var_dump($a);
  // echo "OPERID".$a["operacao"];
  $sql = "SELECT operacoes.valor as valor, operacoes.vencimento, operacoes.nota as nota FROM operacoes left join boletos on boletos.operacao = operacoes.id WHERE operacoes.id = ?";
  $stmt = prepared_query($con, $sql, [$a['operacao']], 'i');
  $operac = $stmt->get_result()->fetch_all(MYSQLI_ASSOC)[0];
  $stmt->close();
  // echo $sql;
  // echo "OPERAC IS".var_dump($operac);
  $trs .= ' 
      <tr>
        <td class="center">' . $operac['nota'] . '</td>
        <td class="left">' . $empresa['razao'] . '</td>
        <td class="center">' . formataCpfCnpj($empresa['cnpj']) . '</td>
        <td class="center">' . date_format(date_create($operac['vencimento']), 'd-m-Y') . '</td>
        <td class="right">R$ ' . number_format($operac['valor'], 2, ',', '.') . '</td>
      </tr>
    ';
...
$html = '
  ...
  <p>
    <span class="under">CESSIONÁRIO:</span> CINQUE CAPITAL SECURITIZADORA S.A, pessoa jurídica de direito privado, devidamente inscrita perante o CNPJ/MF sob o nº 53.177.131/0001-05, com sede na na Rua Coronel Procópio Gomes de
    Oliveira, 430, Sala 08.1, Box 43, Centro, na cidade de Jaraguá do Sul, Estado de Santa
    Catarina, CEP 89.251-200.
    <br><span class="bold">Representante Legal:</span> RAFAEL ENRICO FACHINI
    <br><span class="bold">RG</span> nº 4.805.697
    <br><span class="bold">CPF/MF</span> nº 068.008.369-31
    ...
    <br><span class="bold">Endereço:</span> Rua Curt Brandes, 201, Centro, Pomerode/SC, CEP 89.107-000, Jaraguá do Sul – Estado de Santa Catarina.
    <br><span class="bold">Telefone:</span> (47) 3307-5808
    <br><span class="bold">e-mail:</span> cinque@lawsmart.com.br
  ...
  <p>
    (ii) Para o CESSIONÁRIO - CINQUE CAPITAL SECURITIZADORA  S.A - CNPJ/MF sob o nº 53.177.131/0001-05 na pessoa de seu representante legal, Sr. RAFAEL ENRICO FACHINI
    <br>b.1) e-mail: cinque@lawsmart.com.br
  ...
  <br>CINQUE CAPITAL SECURITIZADORA  S.A
  <br>CNPJ/MF sob o nº 53.177.131/0001-05
  </span>
</p>
...
$contrato = 'contrato_' . $empresa['cnpj'] . '_' . $id . '.pdf';
$mpdf = new \Mpdf\Mpdf();
