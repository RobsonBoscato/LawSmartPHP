	...
	if ($_GET) {
		...
		switch($_GET['f']) {
			...
			case 'operacoes':
				...
				$stmt = prepared_query($con, $sql, [$cnpj, 0, $hoje_mais_10_dias], 'sis');
			...
			case 'baixadas':
				...
				$hoje_mais_10_dias = date("Y-m-d", strtotime($hoje . " +10 days"));
				$sql = "SELECT * FROM operacoes WHERE cnpj=? AND status=? AND vencimento>=? ORDER BY vencimento ASC";
				$stmt = prepared_query($con, $sql, [$cnpj, 1, $hoje_mais_10_dias], 'sis');
				$ope = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
				$stmt->close();
				echo json_encode($ope);			    
			break;
		...
		switch($metodo) {
			...
			case 'detalhes':
				...
				$sql = "INSERT INTO boletos (cnpj, vencimento, valor, operacao, documento, nosso_numero, status,emissao) VALUES (?, ?, ?, ?, ?, ?, ?,NOW())";
				$stmt = prepared_query($con, $sql, [$op['cnpj'], $op['vencimento'], $dados['valor'], $dados['operacao'], $dados['nota'], $nn, 'P'], 'ssdisss');
				$stmt->close();
				...
				// $stmt = prepared_query($con, $sql, [$op['cnpj'], $op['vencimento'], $dados['valorOriginal'], $dados['operacao'], $dados['nota'], $nn, 'P',$dataDia], 'ssdisss');
				// $stmt->close();
			break;
		}
	}
