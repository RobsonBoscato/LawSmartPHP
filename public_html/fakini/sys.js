...
function buscar() {
	...
	function montaHtml() {
		...
		if (obj.fornecedor.operacoes.length > 0) {
			obj.fornecedor.operacoes.forEach((op, i) => {
				...
				// Lembre-se de subtrair 1 do mês, pois os meses começam em 0
		...
		// document.querySelector('#limiteDisponivel')
		...
		var limiteDisponivelElement = document.querySelector('#limiteDisponivel');
		var limiteDisponivelText = limiteDisponivelElement.textContent;
		limiteDisponivel = limiteDisponivelText.replace('R$', '').trim();
		limiteDisponivel = limiteDisponivel.replace('.', '').trim();
		limiteDisponivel = limiteDisponivel.replace(',', '.').trim();
	...
	function atualizaAntecipacao() {
		...
		obj.fornecedor.operacoes.forEach(op => {
			console.log('OVER OP.VALOR', op.valor)
			console.log('typeof op.valor', typeof op.valor)
			if (op.antecipar === true) {
				valorOriginal += new Number(op.valor);
				valorJuros += new Number(op.valorDesconto);
				antecipadas++;
			}
		});
		...
		txBoleto = (obj.fornecedor.boleto) === 0 ? new Number(obj.taxas.find(v => v.titulo === 'BOLETO').valor) : new Number(obj.fornecedor.boleto);
		txTed = parseFloat(obj.fornecedor.ted) === 0 ? new Number(obj.taxas.find(v => v.titulo === 'TED').valor) : new Number(obj.fornecedor.ted);
		txTac = parseFloat(obj.fornecedor.tac) === 0 ? new Number(obj.taxas.find(v => v.titulo === 'TAC').valor) : new Number(obj.fornecedor.tac);
		console.log('forneedor TAC', obj.fornecedor.tac)
		console.log('forneedor TED', obj.fornecedor.tac)
		console.log('TX BOLETO', obj.fornecedor.tac)
		valorTaxas += new Number(new Number((txBoleto * antecipadas) + txTed + txTac).toFixed(2));
		console.log('valor taxas is', valorTaxas)
		...
		// else { 
		console.log('fornecedor is >>>', obj.fornecedor)
		btn = `
		...
		if (antecipadas > 0) {
			...
			<td>
				...
				<span class="text-dark fw-bolder d-block fs-5">${moneyFormat.format(new Number(valorOriginal).toFixed(2))}</span>
			</td>
			<td>
				<span class="text-muted fw-bold d-block fs-7">vencimento</span>
			...
			<td>
				...
				<span class="text-dark fw-bolder d-block fs-5">${moneyFormat.format(new Number(valorJuros).toFixed(2))}</span>
			</td>
			<td>
				<span class="text-muted fw-bold d-block fs-7">desconto taxas</span>
				<span class="text-dark fw-bolder d-block fs-5">${moneyFormat.format(new Number(valorTaxas).toFixed(2))}</span>
			</td>
			<td>
				<span class="text-muted fw-bold d-block fs-7">valor antecipação</span>
				<span class="text-dark fw-bolder d-block fs-5">${moneyFormat.format(new Number(valorOriginal - (valorJuros + valorTaxas)).toFixed(2))}*</span>
			</td>
			...
			document.querySelector('#efetuar-antecipadas').addEventListener('click', () => {
				...
				swalWithBootstrapButtons.fire({
					...
					if (result.isConfirmed) {
						...
						swalWithBootstrapButtons.fire({
							title: "Cancelado",
							text: "Operação não foi efetuada.",
							icon: "error"
						});
					}
				});
