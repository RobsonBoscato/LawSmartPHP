												...
												<table class="table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3" id="fluxo_table">
													...
													<tbody>
														<?php
															...
															operacoes.id as id_oper, vencimento, sum(operacoes.valor)  as valor, 
															...
															and (operacoes.confirmada = 1 || antecipadasDetalhes.antecipada is not null and (select status from operacoes oper where oper.id = antecipadasDetalhes.operacao) = 5)

															left join antecipadas on antecipadas.id = antecipadasDetalhes.antecipada
															and (operacoes.confirmada = 1 || antecipadasDetalhes.antecipada is not null and (select status from operacoes oper where oper.id = antecipadasDetalhes.operacao) = 5)

															where operacoes.status not in (0, 1, 6)
																and (operacoes.confirmada = 1 || (postergacoesDetalhes.id_operacao = operacoes.id and (select status from operacoes oper where oper.id  = postergacoesDetalhes.id_operacao ) = 5) || (antecipadasDetalhes.antecipada is not null and (select status from operacoes oper where oper.id = antecipadasDetalhes.operacao) = 5))
															";
															...
															$query .= " group by operacoes.id, antec_id_uniq order by operacoes.vencimento";
															// echo $query;
														...
														<?php } 
															...
															$movimentacoes = mysqli_query($lawsmt, $query_mov);
   														 	while($status == '' && $row = mysqli_fetch_assoc($movimentacoes)) { 
														?>
														<tr>
															<td>
																<a href="#" class="text-dark fw-bolder text-hover-primary d-block mb-1 fs-6"><?php ?></a>
																<span class="text-muted fw-bold text-muted d-block fs-7"><?php ?></span>
															</td>
															<td>
